...
namespace yii\gg\generators\init;
...
/**
 * 给定数据表名,模型名,自动生成对应的model,searchModel,controller
 * @package
 */
class Generator extends \yii\gg\Generator
{
    ...
    public function rules()
    {
        return array_merge(parent::rules(), [
            [
                ['db', 'mns', 'cns', 'tableName', 'modelClass', 'baseClass', 'queryNs', 'queryClass', 'queryBaseClass'],
                'filter',
                'filter' => 'trim'
            ],
            [
            ...
            [['db', 'mns', 'cns', 'tableName', 'baseClass', 'queryNs', 'queryBaseClass'], 'required'],
            [
            ...
            [
                ['mns', 'cns', 'baseClass', 'queryNs', 'queryBaseClass'],
                'match',
                'pattern' => '/^[\w\\\\]+$/',
            ...
            [['db'], 'validateDb'],
            [['mns', 'cns', 'queryNs'], 'validateNamespace'],
    ...
    public function generate()
    {
        ...
        foreach ($this->getTableNames() as $tableName) {
            ...
            $modelSearchClassName = $this->generateClassName($tableName) . 'Search';
            $controllerClassName = $this->generateClassName($tableName) . 'Controller';
            ...
            $modelSearchParams = [
                'rules'            => static::generateSearchRules($tableSchema),
                'labels'           => $this->generateLabels($tableSchema),
                'searchAttributes' => static::getSearchAttributes($tableSchema),
                'searchConditions' => static::generateSearchConditions($tableSchema),
            ];

            $files[] = new CodeFile(
                Yii::getAlias('@' . str_replace('\\', '/', $this->mns)) . '/' . $modelClassName . '.php',
                $this->render('model.php', $modelParams)
            );

            $files[] = new CodeFile(
                Yii::getAlias('@' . str_replace('\\', '/', $this->mns)) . '/' . $modelSearchClassName . '.php',
                $this->render('search.php', $modelSearchParams)
            );


            $files[] = new CodeFile(
                Yii::getAlias('@' . str_replace('\\', '/', $this->cns)) . '/' . $controllerClassName . '.php',
                $this->render('controller.php', $controllerParams)
            );
            ...
            foreach ($db->getSchema()->getTableSchemas($schemaName) as $table) {
                ...
                foreach ($table->foreignKeys as $refs) {
                    ...
                    if ($refTableSchema === null) {
                        // Foreign key could point to non-existing table: https://github.com/yiisoft/yii2-gg/issues/34
                        continue;
                    }
                    unset($refs[0]);
                    $fks = array_keys($refs);
